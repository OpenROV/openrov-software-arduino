...
namespace
{
    ...
    void initBNO055()
    {
        ...
        Serial.print( "BNO055.Initial_Mode:" );
        Serial.print( mode );
        Serial.println( ';' );
...
void CBNO055::Initialize()
{
    ...
    Serial.println( ';' );
    Serial.print( "BNO055.WHO_AM_I:" );
    Serial.print( BNO055_CHIP_ID );
    Serial.println( ';' );
    Serial.print( "BNO055.IAM_160:" );
    Serial.print( c );
    Serial.println( ';' );
    ...
    if( c == 0xA0 )  // BNO055 WHO_AM_I should always be 0xA0
    {
        ...
        Serial.print( swmsb, HEX );
        Serial.print( "." );
        Serial.print( swlsb, HEX );
        Serial.println( ";" );
...
void CBNO055::Update( CCommand& command )
{
    if( bno055_sample_timer.HasElapsed( 1000 / 75 ) )
    {
        if( !initalized )
        {
            if( report_timer.HasElapsed( 30000 ) )
            {
                Initialize();
            }
        ...
        NDataManager::m_navData.PITC = bPitch;
        NDataManager::m_navData.ROLL = bRoll;
        NDataManager::m_navData.YAW = bYaw;
        NDataManager::m_navData.HDGD = bYaw;

        // TEST: Controller test code

        // Normalize
        NDataManager::m_controllerData.roll = NORMALIZE_ANGLE( bRoll );
        NDataManager::m_controllerData.pitch = NORMALIZE_ANGLE( bPitch );
        NDataManager::m_controllerData.yaw = NORMALIZE_ANGLE( bYaw );
    }
}
#endif
