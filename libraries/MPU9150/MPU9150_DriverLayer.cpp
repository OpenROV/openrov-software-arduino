...
#define i2c_write   I2C0.WriteRegisterBytes
//!I2Cdev::writeBytes
#define i2c_read    I2C0.ReadRegisterBytes
//!I2Cdev::readBytes
...
static int set_int_enable( unsigned char enable )
{
	...
	if( st->chip_cfg.dmp_on )
	{
		...
		if( i2c_write( st->hw->addr, st->reg->int_enable, &tmp, 1 ) )
		{
			return -1;
		}
...
int mpu_reg_dump( void )
{
	...
	for( ii = 0; ii < st->hw->num_reg; ii++ )
	{
		...
		if( i2c_read( st->hw->addr, ii, &data, 1 ) )
		{
			return -1;
		}
...
int mpu_read_reg( unsigned char reg, unsigned char* data )
{
	...
	return i2c_read( st->hw->addr, reg, data, 1 );
}
...
int mpu_init( struct int_param_s* int_param )
{
	...
	if( i2c_write( st->hw->addr, st->reg->pwr_mgmt_1, data, 1 ) )
	{
		return -1;
	}
	...
	if( i2c_read( st->hw->addr, st->reg->accel_offs, data, 6 ) )
	{
		return -3;
	}
	...
	if( rev )
	{
		...
		{
#ifdef MPU_DEBUG
			Serial.print( "Unsupported software product rev: " );
			Serial.println( rev );
#endif
			return -4;
		}
	...
	{
		if( i2c_read( st->hw->addr, st->reg->prod_id, data, 1 ) )
		{
			return -5;
		}
...
#define MPU6500_MEM_REV_ADDR    (0x17)
	...
	if( i2c_write( st->hw->addr, st->reg->accel_cfg2, data, 1 ) )
	{
		return -1;
	}
	...
	{
#ifdef MPU_DEBUG
		Serial.print( "Setup compass failed: " );
...
int mpu_lp_accel_mode( unsigned char rate )
{
	...
	if( !rate )
	{
		...
		if( i2c_write( st->hw->addr, st->reg->pwr_mgmt_1, tmp, 2 ) )
		{
			return -1;
		}
	...
	if( i2c_write( st->hw->addr, st->reg->lp_accel_odr, tmp, 1 ) )
	{
		return -1;
	}
	...
	if( i2c_write( st->hw->addr, st->reg->pwr_mgmt_1, tmp, 1 ) )
	{
		return -1;
	}
...
int mpu_get_gyro_reg( short* data, unsigned long* timestamp )
{
	...
	if( i2c_read( st->hw->addr, st->reg->raw_gyro, tmp, 6 ) )
	{
		return -1;
	}
...
int mpu_get_accel_reg( short* data, unsigned long* timestamp )
{
	...
	if( i2c_read( st->hw->addr, st->reg->raw_accel, tmp, 6 ) )
	{
		return -1;
	}
...
int mpu_get_temperature( long* data, unsigned long* timestamp )
{
	...
	if( i2c_read( st->hw->addr, st->reg->temp, tmp, 2 ) )
	{
		return -1;
	}
...
int mpu_set_accel_bias( const long* accel_bias )
{
	...
	if( i2c_read( st->hw->addr, 3, data, 3 ) )
	{
		return -1;
	}
	...
	if( i2c_read( st->hw->addr, 0x06, data, 6 ) )
	{
		return -1;
	}
	...
	if( i2c_write( st->hw->addr, 0x06, data, 6 ) )
	{
		return -1;
	}
...
int mpu_reset_fifo( void )
{
	...
	if( i2c_write( st->hw->addr, st->reg->int_enable, &data, 1 ) )
	{
		return -1;
	}

	if( i2c_write( st->hw->addr, st->reg->fifo_en, &data, 1 ) )
	{
		return -1;
	}

	if( i2c_write( st->hw->addr, st->reg->user_ctrl, &data, 1 ) )
	{
		return -1;
	}
	...
	else
	{
		...
		if( i2c_write( st->hw->addr, st->reg->fifo_en, &st->chip_cfg.fifo_enable, 1 ) )
		{
			return -1;
		}
	}
...
int mpu_set_gyro_fsr( unsigned short fsr )
{
	...
	if( i2c_write( st->hw->addr, st->reg->gyro_cfg, &data, 1 ) )
	{
		return -1;
	}
...
int mpu_set_accel_fsr( unsigned char fsr )
{
	...
	if( i2c_write( st->hw->addr, st->reg->accel_cfg, &data, 1 ) )
	{
		return -1;
	}
...
int mpu_set_lpf( unsigned short lpf )
{
	...
	if( i2c_write( st->hw->addr, st->reg->lpf, &data, 1 ) )
	{
		return -1;
	}
...
int mpu_set_sample_rate( unsigned short rate )
{
	...
	else
	{
		...
		if( i2c_write( st->hw->addr, st->reg->rate_div, &data, 1 ) )
		{
			return -1;
		}
...
int mpu_set_compass_sample_rate( unsigned short rate )
{
	...
	if( i2c_write( st->hw->addr, st->reg->s4_ctrl, &div, 1 ) )
	{
		return -1;
	}
...
int mpu_set_sensors( unsigned char sensors )
{
	...
	if( i2c_write( st->hw->addr, st->reg->pwr_mgmt_2, &data, 1 ) )
	{
		st->chip_cfg.sensors = 0;
		return -1;
	}
	...
	if( i2c_read( st->hw->addr, st->reg->user_ctrl, &user_ctrl, 1 ) )
	{
		return -1;
	}
	...
	if( i2c_write( st->hw->addr, st->reg->s1_do, &data, 1 ) )
	{
		return -1;
	}

	/* Enable/disable I2C master mode. */
	if( i2c_write( st->hw->addr, st->reg->user_ctrl, &user_ctrl, 1 ) )
	{
		return -1;
	}
...
int mpu_get_int_status( short* status )
{
	...
	if( i2c_read( st->hw->addr, st->reg->dmp_int_status, tmp, 2 ) )
	{
		return -1;
	}
                   ...
                   unsigned char* sensors, unsigned char* more )
{
	...
	if( i2c_read( st->hw->addr, st->reg->fifo_count_h, data, 2 ) )
	{
		return -4;
	}
	...
	if( fifo_count > ( st->hw->max_fifo >> 1 ) )
	{
		...
		if( i2c_read( st->hw->addr, st->reg->int_status, data, 1 ) )
		{
			return -5;
		}
	...
	if( i2c_read( st->hw->addr, st->reg->fifo_r_w, data, packet_size ) )
	{
		return -7;
	}
                          ...
                          unsigned char* more )
{
	...
	if( i2c_read( st->hw->addr, st->reg->fifo_count_h, tmp, 2 ) )
	{
		return -3;
	}
	...
	{
		/* FIFO is 50% full, better check overflow bit. */
		if( i2c_read( st->hw->addr, st->reg->int_status, tmp, 1 ) )
		{
			return -5;
		}
	...
	if( i2c_read( st->hw->addr, st->reg->fifo_r_w, data, length ) )
	{
		return -7;
	}
...
int mpu_set_bypass( unsigned char bypass_on )
{
	...
	if( bypass_on )
	{
		if( i2c_read( st->hw->addr, st->reg->user_ctrl, &tmp, 1 ) )
		{
			return -1;
		}
		...
		if( i2c_write( st->hw->addr, st->reg->user_ctrl, &tmp, 1 ) )
		{
			return -1;
		}
		...
		if( i2c_write( st->hw->addr, st->reg->int_pin_cfg, &tmp, 1 ) )
		{
			return -1;
		}
	}
...
static int get_accel_prod_shift( float* st_shift )
{
	...
	if( i2c_read( st->hw->addr, 0x0D, tmp, 4 ) )
	{
		return 0x07;
	}
...
static int gyro_self_test( long* bias_regular, long* bias_st )
{
	...
	if( i2c_read( st->hw->addr, 0x0D, tmp, 3 ) )
	{
		return 0x07;
	}
...
static int compass_self_test( void )
{
	...
	if( i2c_write( st->chip_cfg.compass_addr, AKM_REG_CNTL, tmp, 1 ) )
	{
		return 0x07;
	}
	...
	if( i2c_write( st->chip_cfg.compass_addr, AKM_REG_ASTC, tmp, 1 ) )
	{
		goto AKM_restore;
	}
	...
	do
	{
		...
		if( i2c_read( st->chip_cfg.compass_addr, AKM_REG_ST1, tmp, 1 ) )
		{
			goto AKM_restore;
		}
	...
	if( i2c_read( st->chip_cfg.compass_addr, AKM_REG_HXL, tmp, 6 ) )
	{
		goto AKM_restore;
	}
...
AKM_restore:
	...
	i2c_write( st->chip_cfg.compass_addr, AKM_REG_ASTC, tmp, 1 );
	tmp[0] = SUPPORTS_AK89xx_HIGH_SENS;
	i2c_write( st->chip_cfg.compass_addr, AKM_REG_CNTL, tmp, 1 );
	mpu_set_bypass( 0 );
	return result;
}
#endif
...
static int get_st_biases( long* gyro, long* accel, unsigned char hw_test )
{
	...
	if( i2c_write( st->hw->addr, st->reg->pwr_mgmt_1, data, 2 ) )
	{
		return -1;
	}
	...
	if( i2c_write( st->hw->addr, st->reg->i2c_mst, data, 1 ) )
	{
		return -1;
	}
	...
	for( ii = 0; ii < packet_count; ii++ )
	{
		...
		if( i2c_read( st->hw->addr, st->reg->fifo_r_w, data, MAX_PACKET_LENGTH ) )
		{
			return -1;
		}
                   ...
                   unsigned char* data )
{
	...
	if( i2c_write( st->hw->addr, st->reg->bank_sel, tmp, 2 ) )
	{
		return -4;
	}

	if( i2c_write( st->hw->addr, st->reg->mem_r_w, data, length ) )
	{
		return -5;
	}
                  ...
                  unsigned char* data )
{
	...
	if( i2c_read( st->hw->addr, st->reg->mem_r_w, data, length ) )
	{
		return -1;
	}
...
#define LOAD_CHUNK  (16)
	...
	for( ii = 0; ii < length; ii += this_write )
	{
		...
		{
#ifdef MPU_DEBUG
			Serial.print( "Firmware compare failed addr " );
			Serial.println( ii );
	...
	if( i2c_write( st->hw->addr, st->reg->prgm_start_h, tmp, 2 ) )
	{
		return -6;
	}
...
int mpu_set_dmp_state( unsigned char enable )
{
	...
	if( enable )
	{
		...
		tmp = 0;
		i2c_write( st->hw->addr, 0x23, &tmp, 1 );
...
static int setup_compass( void )
{
	...
	{
		int result;
		result = i2c_read( akm_addr, AKM_REG_WHOAMI, data, 1 );
	...
	if( i2c_write( st->chip_cfg.compass_addr, AKM_REG_CNTL, data, 1 ) )
	{
		return -2;
	}
	...
	if( i2c_read( st->chip_cfg.compass_addr, AKM_REG_ASAX, data, 3 ) )
	{
		return -4;
	}
...
#ifdef MPU_DEBUG
	Serial.print( "Compass sens: " );
	Serial.print( st->chip_cfg.mag_sens_adj[0] );
	Serial.print( " " );
	Serial.print( st->chip_cfg.mag_sens_adj[1] );
	Serial.print( " " );
	Serial.print( st->chip_cfg.mag_sens_adj[2] );
	Serial.println();
#endif
	...
	if( i2c_write( st->hw->addr, st->reg->s0_addr, data, 1 ) )
	{
		return -7;
	}
	...
	if( i2c_write( st->hw->addr, st->reg->s0_reg, data, 1 ) )
	{
		return -8;
	}
	...
	if( i2c_write( st->hw->addr, st->reg->s0_ctrl, data, 1 ) )
	{
		return -9;
	}
	...
	if( i2c_write( st->hw->addr, st->reg->s1_addr, data, 1 ) )
	{
		return -10;
	}
	...
	if( i2c_write( st->hw->addr, st->reg->s1_reg, data, 1 ) )
	{
		return -11;
	}
	...
	if( i2c_write( st->hw->addr, st->reg->s1_ctrl, data, 1 ) )
	{
		return -12;
	}
	...
	if( i2c_write( st->hw->addr, st->reg->i2c_delay_ctrl, data, 1 ) )
	{
		return -14;
	}
	...
	if( i2c_write( st->hw->addr, st->reg->yg_offs_tc, data, 1 ) )
	{
		return -15;
	}
...
int mpu_get_compass_reg( short* data, unsigned long* timestamp )
{
	...
	if( i2c_read( st->chip_cfg.compass_addr, AKM_REG_ST1, tmp, 8 ) )
	{
		return -2;
	}
	...
	if( i2c_write( st->chip_cfg.compass_addr, AKM_REG_CNTL, tmp + 8, 1 ) )
	{
		return -3;
	}
	...
	if( i2c_read( st->hw->addr, st->reg->raw_compass, tmp, 8 ) )
	{
		return -4;
	}
                             ...
                             unsigned char lpa_freq )
{
	...
	if( lpa_freq )
	{
		...
		if( i2c_write( st->hw->addr, st->reg->motion_thr, data, 2 ) )
		{
			goto lp_int_restore;
		}
		...
		if( i2c_write( st->hw->addr, st->reg->user_ctrl, data, 3 ) )
		{
			goto lp_int_restore;
		}
		...
		if( i2c_write( st->hw->addr, st->reg->motion_thr, data, 1 ) )
		{
			goto lp_int_restore;
		}
		...
		if( i2c_write( st->hw->addr, st->reg->lp_accel_odr, data, 1 ) )
		{
			goto lp_int_restore;
		}
		...
		if( i2c_write( st->hw->addr, st->reg->accel_intel, data, 1 ) )
		{
			goto lp_int_restore;
		}
