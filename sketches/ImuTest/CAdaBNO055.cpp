...
CAdaBNO055::CAdaBNO055( TwoWire *wireInterfaceIn, int32_t sensorIdIn, uint8_t addressIn )
{
...
bool CAdaBNO055::Initialize()
{
	if( m_pWire == nullptr )
	{
		Serial.println( "BNO055 not attached to a wire interface! Set one before trying to initialize!" );
		return false;
	}
...
void CAdaBNO055::SetWireInterface( TwoWire* wireInterfaceIn )
{
	m_pWire = wireInterfaceIn;
}
...
bool CAdaBNO055::ReadByte( ERegisterAddress addressIn, uint8_t& dataOut )
{
  ...
  m_pWire->endTransmission();
  m_pWire->requestFrom(m_i2cAddress, (byte)1);
...
bool CAdaBNO055::ReadNBytes(ERegisterAddress addressIn, uint8_t* dataOut, uint8_t byteCountIn)
{
  ...
  m_pWire->endTransmission();
  m_pWire->requestFrom(m_i2cAddress, (byte)byteCountIn);
