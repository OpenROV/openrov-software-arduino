...
public:
	...
	CAdaBNO055( TwoWire *wireInterfaceIn, int32_t sensorIdIn = -1, uint8_t addressIn = BNO055_ADDRESS_B );

	bool Initialize();
	bool Reset();
	bool SetMode( EOpMode modeIn );
	bool SetExternalCrystalUse( boolean shouldUseIn );
	bool SetPowerMode( EPowerMode powerModeIn );
	bool SetSelectedRegisterPage( byte pageIdIn );
	bool SetUpUnitsAndOrientation();
	bool VerifyChipId();
	bool GetCalibration();
	bool GetRevInfo( TRevisionInfo& revInfoOut );
	bool GetVector( EVectorType vectorTypeIn, imu::Vector<3>& vectorOut );
	...
	void SetWireInterface( TwoWire *wireInterfaceIn );
...
private:
	bool ReadByte( ERegisterAddress addressIn, uint8_t& dataOut );
	bool ReadNBytes( ERegisterAddress addressIn, uint8_t* dataOut, uint8_t byteCountIn );
	bool WriteByte( ERegisterAddress addressIn, uint8_t dataIn );
	...
	TCalibrationOffsets m_accOffsets;
	TCalibrationOffsets m_gyroOffsets;
	TCalibrationOffsets m_magOffsets;
